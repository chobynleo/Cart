...
contract ERC721BasicToken is SupportsInterfaceWithLookup, ERC721Basic {
  ...
  */
    ...
    public override
  {
    ...
    console.log("transferFrom id:", _tokenId);
...
contract ERC721Token is SupportsInterfaceWithLookup, ERC721BasicToken, ERC721 {
  ...
  /**
   * @dev return all token ID at a given address of user
   * @param _owner address owning the tokens list to be accessed
   * @return all uint256 token ID list of this tokens
   */
  function tokenByOwner(address _owner) public view returns (uint256 [] memory) {
    
    uint256 length = ownedTokens[_owner].length;
    uint256 [] memory tokenIDArray = new uint256 [](length);
    for(uint256 i = 0; i < length; i++){
      tokenIDArray[i] = ownedTokens[_owner][i];
    }
    return tokenIDArray;
  }
...
contract NFTMock is ERC721Token {
